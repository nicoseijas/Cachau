...
**Multiple processes sharing one cache directory** is safe for correctness but not yet deduplicated: writes are atomic (per-writer temp file, `os.replace`, directory fsync), so concurrent workers can never corrupt the store or read a torn entry — but single-flight coalescing is per-process, so N workers missing the same expensive key all compute it and the last write wins. Wasted compute, never a wrong value. Cross-process single-flight is an open design problem ([#35](https://github.com/nicoseijas/Cachau/issues/35)).
